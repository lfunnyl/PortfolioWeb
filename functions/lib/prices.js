...
exports.priceForecast = exports.priceSignals = exports.priceChart = exports.priceHistorical = exports.pricesBulk = void 0;
/**
...
function getYahooTicker(assetId) {
...
exports.pricesBulk = (0, https_1.onRequest)({ region: 'europe-west1', memory: '256MiB', timeoutSeconds: 30 }, async (req, res) => {
    setCors(res);
    ...
    try {
        ...
        for (const aid of assetIds) {
            ...
            else if (ticker && (ticker.endsWith('-USD') ||
...
exports.priceHistorical = (0, https_1.onRequest)({ region: 'europe-west1', memory: '256MiB', timeoutSeconds: 30 }, async (req, res) => {
    setCors(res);
    if (req.method === 'OPTIONS') {
        res.status(204).send('');
        return;
    ...
    try {
        ...
        dtStart.setDate(dtStart.getDate() - 5);
        const dtEnd = new Date(dt);
        dtEnd.setDate(dtEnd.getDate() + 5);
...
exports.priceChart = (0, https_1.onRequest)({ region: 'europe-west1', memory: '256MiB', timeoutSeconds: 30 }, async (req, res) => {
    setCors(res);
    if (req.method === 'OPTIONS') {
        res.status(204).send('');
        return;
...
exports.priceSignals = (0, https_1.onRequest)({ region: 'europe-west1', memory: '512MiB', timeoutSeconds: 60 }, async (req, res) => {
    setCors(res);
    if (req.method === 'OPTIONS') {
        res.status(204).send('');
        return;
    ...
    try {
        ...
        let macdSignal = 'NÖTR';
        if (macdLine[n] > signalLine[n] && macdLine[n1] <= signalLine[n1])
            macdSignal = 'GÜÇLÜ AL (Yukarı Kesişim)';
        else if (macdLine[n] < signalLine[n] && macdLine[n1] >= signalLine[n1])
            macdSignal = 'GÜÇLÜ SAT (Aşağı Kesişim)';
        else if (histVal > 0)
            macdSignal = 'AL (Trend Pozitif)';
        else
            macdSignal = 'SAT (Trend Negatif)';
        const high52 = Math.max(...closes);
        const low52 = Math.min(...closes);
        let maCross = 'NÖTR';
        if (sma50[n] && sma200[n]) {
            if (sma50[n] > sma200[n] && sma50[n1] <= sma200[n1])
                maCross = '🎉 GOLDEN CROSS (Boğa Piyasası)';
            else if (sma50[n] < sma200[n] && sma50[n1] >= sma200[n1])
                maCross = '⚠️ DEATH CROSS (Ayı Piyasası)';
            else if (sma50[n] > sma200[n])
                maCross = 'Pozitif (50 MA > 200 MA)';
            else
                maCross = 'Negatif (50 MA < 200 MA)';
        }
        ...
        let bbSignal = 'Normal';
        if (bbWidth < 5)
            bbSignal = 'Sıkışma (Sert hareket yaklaşıyor)';
        else if (cur > bbUpper[n])
            bbSignal = 'Üst Bandı Kırdı';
        else if (cur < bbLower[n])
            bbSignal = 'Alt Bandı Kırdı';
        let score = 0;
        if (rsiSignal.includes('AL'))
            score++;
        else if (rsiSignal.includes('SAT'))
            score--;
        if (macdSignal.includes('GÜÇLÜ AL'))
            score += 2;
        else if (macdSignal.includes('AL'))
            score++;
        else if (macdSignal.includes('GÜÇLÜ SAT'))
            score -= 2;
        else if (macdSignal.includes('SAT'))
            score--;
        if (maCross.includes('GOLDEN'))
            score += 3;
        else if (maCross.includes('DEATH'))
            score -= 3;
        else if (maCross.includes('Pozitif'))
            score++;
        else if (maCross.includes('Negatif'))
            score--;
...
exports.priceForecast = (0, https_1.onRequest)({ region: 'europe-west1', memory: '256MiB', timeoutSeconds: 60 }, async (req, res) => {
    setCors(res);
    if (req.method === 'OPTIONS') {
        res.status(204).send('');
        return;
    }
    const { ticker, days: daysStr = '30' } = req.query;
    ...
    try {
        ...
        if (vals.length < 30) {
            res.json({ error: 'Yeterli veri yok (min 30 gün)' });
            return;
        }
        const sesF = (series, alpha, h) => {
            let level = series[0];
